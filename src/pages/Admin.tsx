...
export default function AdminPanel() {
  ...
  const handleFaviconUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (file.size > 800000) return toast.error("Favicon size should be less than 800KB");
    const reader = new FileReader();
    reader.onload = (event) => {
      const base64 = event.target?.result as string;
      if (base64) {
        setFaviconUrl(base64);
        toast.success("Favicon uploaded successfully! Click Apply Branding Config to save.");
      }
    };
  ...
  const handlePreviewImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 2000000) return toast.error("Preview Image size should be less than 2MB");
    const reader = new FileReader();
    reader.onload = (event) => {
      const base64 = event.target?.result as string;
      if (base64) {
        setPreviewImageUrl(base64);
        toast.success("Preview Image uploaded successfully! Click Apply Branding Config to save.");
      }
    };
    reader.onerror = () => toast.error("Failed to read file");
    reader.readAsDataURL(file);
  ...
  const addService = () => {
    ...
    const list = [...(webContent.services || []), {
      ...
      image: newService.image || "/techwin-architecture.webp",
      features: tags
    } as ServiceItem];
    handleSaveContentChange({ ...webContent, services: list });
  ...
  const addIndustry = () => {
    ...
    const list = [...(webContent.industries || []), {
      ...
      image: newIndustry.image || "/techwin-network-hero.webp"
    } as IndustryItem];
    handleSaveContentChange({ ...webContent, industries: list });
            ...
            <TabsContent value="settings" className="space-y-6 focus:outline-none">
              ...
              <Card className="bg-white border-slate-200/80 rounded-2xl shadow-sm max-w-2xl">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-1 mt-4">
                    <label className="text-xs font-semibold text-slate-700">Preview Image URL or Upload</label>
                    <div className="flex gap-2">
                      <Input value={previewImageUrl} onChange={(e) => setPreviewImageUrl(e.target.value)} className="bg-slate-50 border-slate-200 text-xs h-9 rounded-lg flex-1" placeholder="e.g. /hero-preview.png" />
                      <div className="relative">
                        <input type="file" id="preview-upload" accept="image/*" className="hidden" onChange={handlePreviewImageUpload} />
                        <Button type="button" variant="outline" onClick={() => document.getElementById("preview-upload")?.click()} className="border-slate-200 bg-white hover:bg-slate-50 text-xs h-9 shadow-sm">
                          <Upload className="w-3.5 h-3.5 mr-1" />
                          Upload Image
                        </Button>
                      </div>
                  ...
                  <div className="flex justify-end pt-2 mt-4">
                    <Button size="sm" onClick={() => handleSaveContentChange({
                      ...webContent,
                      branding: { companyName, brandName, domain, twitterHandle, logoUrl: "", faviconUrl, previewImageUrl }
              ...
              <Card className="bg-white border-slate-200/80 rounded-2xl shadow-sm max-w-2xl">
                <CardHeader>
                  <CardTitle className="text-sm font-bold text-slate-900">Services Gateway</CardTitle>
                  <CardDescription>Configure analytics and system operations</CardDescription>
                </CardHeader>
                    ...
                    <Card className="bg-slate-50/30 border-slate-200 border-dashed rounded-xl mt-4">
                      ...
                      <CardContent className="space-y-3 px-4 pb-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <Input value={newContactMethod.contact} onChange={(e) => setNewContactMethod({ ...newContactMethod, contact: e.target.value })} className="bg-white border-slate-200 text-xs h-9" placeholder="Detail value (e.g. hello@techwensys.com)" />
