        ...
        <div className="container mx-auto px-4 lg:px-8 max-w-7xl">
          <div className="space-y-16 md:space-y-24">
            {productsList.map((product, index) => {
              ...
              const IconComponent = getLucideIcon(product.iconName);
              return (
                <div
                ...
                >
                  <div className={`lg:col-span-7 space-y-6 ${index % 2 === 1 ? "lg:order-2" : ""}`}>
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-neutral-100 flex items-center justify-center text-[#1e6fd9] rounded-none">
                        <IconComponent className="w-4 h-4" strokeWidth={1.5} />
                      </div>
                      <p className="text-[#1e6fd9] font-bold text-xs uppercase tracking-wider">{product.subtitle}</p>
                    </div>
                    ...
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {product.features?.map((feature: string) => (
                        <li key={feature} className="flex items-center gap-3 text-[#555555] text-xs">
                          <CheckCircle className="w-4 h-4 text-[#1e6fd9] shrink-0 rounded-none" />
                          <span>{feature}</span>
                        </li>
                      ))}
                    </ul>
